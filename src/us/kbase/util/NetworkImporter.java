...
public class NetworkImporter {
	...
	public NetworkImporter (String taxId, String workDir, String wsId, String token) throws Exception{
		this.ncbiId = taxId;
		if (workDir == null) {
	...
	public InteractionSet ImportOperonsFile (){
		...
		for (String line : fileData){
			...
			Interaction interaction = new Interaction().withEntity1Id(fields[2]).withEntity2Id(fields[3]).withId("kb|interaction." + startKbaseId.toString()).withType("operon");
			startKbaseId++;
	...
	public InteractionSet ImportStringFile (){
		...
		for (String line : fileData){
			System.out.println(line);
			String[] fields = line.split("\t");
			Interaction interaction = new Interaction().withEntity1Id(fields[0]).withEntity2Id(fields[1]).withId("kb|interaction." + startKbaseId.toString()).withType("string");
			startKbaseId++;
			Map<String, Double> scores = new HashMap<String, Double>();
